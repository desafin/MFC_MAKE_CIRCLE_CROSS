...
BEGIN_MESSAGE_MAP(CgPrjDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_MAKE_CIRCLE, &CgPrjDlg::OnBnClickedMakeCircle)

	//ON_EN_CHANGE(IDC_EDIT_CIRCLE_RAD, &CgPrjDlg::OnEnChangeEditCircleRad)
END_MESSAGE_MAP()
...
void CgPrjDlg::OnBnClickedMakeCircle()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.

	int editRadius = GetDlgItemInt(IDC_EDIT_CIRCLE_RAD);

	std::cout << editRadius << std::endl;


	CClientDC dc(this);

	// 원의 색상을 설정하기 위해 브러시 객체를 생성
	CBrush brush;
		brush.CreateSolidBrush(RGB(255, 0, 0)); // 빨간색 원

	// 원의 외곽선 색상을 설정하기 위해 펜 객체를 생성
	CPen pen;
	pen.CreatePen(PS_SOLID, 5, RGB(255, 255, 0)); // 노란색 펜, PS_SOLID는 실선 스타일
	...
	int min_value = 100;  // 원하는 범위의 최소값
	int max_value = 400; // 원하는 범위의 최대값
	
						 
						 
	// 원의 중심 좌표 계산
	int centerX = min_value + rand() % (max_value - min_value + 1); // 중심 x 좌표
	int centerY = min_value + rand() % (max_value - min_value + 1); // 중심 y 좌표
	int radius = editRadius;  // 원의 반지름


					
	dc.Ellipse(centerX - radius, centerY - radius, centerX + radius, centerY + radius);  // 원 그리기
...
void CgPrjDlg::OnEnChangeEditCircleRad()
{
